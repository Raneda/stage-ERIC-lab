...
for i = 1:n
    cles_locales{i} = struct('paire',{},'cle',{},'hash',{});
end
...
% =========================================================
% AFFICHAGE FINAL — RS avec communications sécurisées
% =========================================================
figure('Name','RS — Communications sécurisées établies',...
...
for k = 1:nb_groupes
    c = couleurs(k,:);
    h_leg(k) = scatter(nan,nan,160,'o','MarkerEdgeColor',c,...
                       'MarkerFaceColor',c,'DisplayName',sprintf('Groupe %d',k));
end

plot(nan,nan,'-','Color',[0.05 0.65 0.35],'LineWidth',3,...
     'DisplayName','Communication sécurisée');

xlim([0 breadth]); ylim([0 breadth]);
title(sprintf('RS — %d communication(s) sécurisée(s)', reponse.nb_paires));
